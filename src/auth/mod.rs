...
impl AuthService {
    pub(crate) fn new_default() -> Self {
        let base_dir = AppPaths::resolve()
            .map(|paths| paths.base_dir)
            .unwrap_or_else(|_| PathBuf::from(".simpleclaw"));
        let path = base_dir.join("auth.json");
        Self {
            store: AuthStore::new(path),
...
impl AuthStore {
    ...
    async fn get_profile(
        ...
        let data = self.load()?;
        let id = match profile_override {
            Some(profile_name) => profile_id(provider, &normalize_profile_name(profile_name)?),
    ...
    async fn upsert_oauth_profile(
        ...
        if set_active {
            data.active_profiles.insert(provider.to_owned(), id);
        }
    ...
    async fn remove_profile(
        ...
        let mut data = self.load()?;
        let profile_id = if let Some(profile_name) = profile_name {
            ...
            .unwrap_or(false)
        {
                ...
                .min_by(|left, right| left.profile_name.cmp(&right.profile_name))
            {
                data.active_profiles
                    .insert(provider.to_owned(), next_active.id.clone());
            }
        }
    ...
    fn load(&self) -> Result<AuthData, FrameworkError> {
    ...
    fn save(&self, data: &AuthData) -> Result<(), FrameworkError> {
...
mod tests {
  ...
  "active_profiles": {
    "openai_codex": "openai_codex:default"
        ...
        let data = store.load().expect("legacy auth should parse");
