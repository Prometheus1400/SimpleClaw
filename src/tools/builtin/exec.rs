...
const SANDBOX_EXEC_TIMEOUT_SECS: u64 = 120;
...
impl Tool for ExecTool {
    ...
    async fn execute(
        ...
        if args.background {
            let (session_id, handle) = if ctx.sandbox.enabled {
                ctx.process_manager
                    .spawn_sandboxed(args.command.trim(), &ctx.workspace_root, &ctx.sandbox)
                    .await?
            } else {
                ctx.process_manager
        ...
        let result = if ctx.sandbox.enabled {
            exec_with_sandbox_runtime(args.command.trim(), &ctx.workspace_root, &ctx.sandbox)
                .await?
        } else {
...
async fn exec_with_sandbox_runtime(
    ...
    let output = timeout(
        Duration::from_secs(SANDBOX_EXEC_TIMEOUT_SECS),
        runner.output(),
    )
    .await
    .map_err(|_| {
